...
_FileSizeExceeded = _FileSizeExceededError
...
async def _process_direct_once(
    agent_loop: Any,
    *,
    user_content: str,
    media_paths: list[str],
...
def _save_base64_data_url(data_url: str, media_dir: Path) -> str | None:
    ...
    matched = _DATA_URL_RE.match(data_url)
    if not matched:
        return None
    mime_type, b64_payload = matched.group(1), matched.group(2)
    try:
...
async def handle_chat_completions(request: web.Request) -> web.Response:
    ...
    try:
        if content_type.startswith("multipart/"):
            user_content, media_paths, session_id, requested_model = await _parse_multipart(request)
        else:
            try:
            ...
            user_content, media_paths = _parse_json_content(body)
    ...
    logger.info(
        "API request session_key={} media={} content={} stream={}",
        session_key,
        len(media_paths),
        user_content[:80],
        stream,
    ...
    if stream:
        ...
        async def _on_stream_end(*_args: Any, **_kwargs: Any) -> None:
        ...
        try:
            while True:
                ...
                if token is None:
                    break
                await response.write(_sse_chunk(token, model_name, chunk_id))
            if not stream_failed:
                await response.write(_sse_chunk("", model_name, chunk_id, finish_reason="stop"))
    ...
    fallback = AgentRunner._user_facing_error_message(
    ...
    try:
        async with session_lock:
            try:
                response = await _process_direct_once(
                    agent_loop,
                    user_content=user_content,
                    media_paths=media_paths,
                    session_key=session_key,
                ...
                if not response_text or not response_text.strip():
                    ...
                    retry_response = await _process_direct_once(
                        agent_loop,
                        user_content=user_content,
                        media_paths=media_paths,
                        session_key=session_key,
