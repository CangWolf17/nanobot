...
def build_status_content(
    *,
    ...
    if harness_summary and interrupt_line:
        harness_status = interrupt_line.split("\u2014", 1)[0].split("/", 1)[0].strip().lower()
        interrupt_label = "Interrupt" if "interrupt" in harness_status else "Harness"
    return "\n".join(
        [
            ...
            *([f"\u2757 {interrupt_label}: {interrupt_line}"] if interrupt_line else []),
            f"\u23f1 Uptime: {uptime}",
        ]
    )
