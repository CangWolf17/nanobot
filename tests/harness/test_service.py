...
def test_service_start_goal_creates_or_resumes_active_harness(tmp_path) -> None:
...
def test_service_start_workflow_reuse_refreshes_goal_memory_and_return_to(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
...
def test_service_start_goal_while_cleanup_is_active_creates_work_harness(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
...
def test_service_renderers_use_canonical_store_views(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
...
def test_auto_continue_decision_falls_back_to_session_bound_harness(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
    ...
    assert first.active_harness_id != second.active_harness_id
...
def test_redirect_after_interrupt_resumes_active_harness_from_service(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
...
def test_redirect_after_interrupt_falls_back_to_session_bound_harness(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
...
def test_structured_harness_apply_updates_store_and_generates_closeout(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
...
def test_structured_harness_apply_targets_bound_harness_not_current_active(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
...
def test_structured_harness_apply_falls_back_to_session_bound_harness(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
...
def test_service_projection_status_reports_store_path_and_last_sync(tmp_path) -> None:
    service = HarnessService.for_workspace(tmp_path)
