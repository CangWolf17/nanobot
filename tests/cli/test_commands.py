...
def test_heartbeat_retains_recent_messages_by_default():
    ...
    assert config.gateway.heartbeat.startup_notify.enabled is False
    assert config.gateway.heartbeat.startup_notify.channel == ""
    assert config.gateway.heartbeat.startup_notify.chat_id == ""
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    assert seen["task_context"] == "Remind me to stretch."
...
def test_gateway_cron_job_forwards_response_metadata_and_completion_notice(
    monkeypatch, tmp_path: Path
) -> None:
    config_file = tmp_path / "instance" / "config.json"
    config_file.parent.mkdir(parents=True)
    ...
    job = CronJob(
        ...
        payload=CronPayload(
            ...
            channel="feishu",
            to="ou_user",
            completion_notice_text="来自 payload 的文案",
    ...
    assert response == "正文"
    assert seen["process_direct_metadata"] == {
        "_origin_sender_id": "ou_creator",
        "_completion_notice_text": "来自 payload 的文案",
    }
    assert bus.publish_outbound.await_count == 2
    first = bus.publish_outbound.await_args_list[0].args[0]
    second = bus.publish_outbound.await_args_list[1].args[0]
    assert first.content == "正文"
...
def test_gateway_cron_weather_task_uses_weather_brief_workspace_route(
    monkeypatch, tmp_path: Path
) -> None:
    config_file = tmp_path / "instance" / "config.json"
    config_file.parent.mkdir(parents=True)
    ...
    monkeypatch.setattr("nanobot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nanobot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("nanobot.bus.queue.MessageBus", lambda: MagicMock())
    monkeypatch.setattr("nanobot.session.manager.SessionManager", lambda _workspace: object())
...
def test_gateway_startup_notify_publishes_online_notice(
    monkeypatch, tmp_path: Path
) -> None:
    config_file = _write_instance_config(tmp_path)
    config = Config()
    ...
    config.providers.custom.api_base = "https://runtime.test/v1"
    config.gateway.heartbeat.startup_notify.enabled = True
    config.gateway.heartbeat.startup_notify.channel = "telegram"
    config.gateway.heartbeat.startup_notify.chat_id = "user-1"
    bus = MagicMock()
    bus.publish_outbound = AsyncMock()
    ...
    _patch_cli_command_runtime(
        ...
        session_manager=lambda _workspace: MagicMock(list_sessions=lambda: []),
    )
    monkeypatch.setattr("nanobot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nanobot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("nanobot.cron.service.CronService", _FakeCronService)
    ...
    assert result.exit_code == 0
    bus.publish_outbound.assert_any_await(
        OutboundMessage(
            channel="telegram",
            chat_id="user-1",
            content="nanobot 已上线（model: test-model, provider: runtime.test, heartbeat: 1800s）",
            metadata={"render_as": "interactive"},
        )
    )


def test_gateway_heartbeat_exec_merges_workspace_bridge_prepared_input(
    monkeypatch, tmp_path: Path
) -> None:
    config_file = _write_instance_config(tmp_path)
    config = Config()
    ...
    class _FakeAgentLoop:
        def __init__(self, **_kwargs) -> None:
            ...
            self.sessions.get_or_create.return_value = _FakeSession()
            self.sessions.save.return_value = None

        async def process_direct(self, content, **kwargs):
            seen["process_direct"] = {"content": content, **kwargs}
            return OutboundMessage(channel="telegram", chat_id="user-1", content="ok")
    ...
    class _FakeHeartbeatService:
        def __init__(self, **kwargs) -> None:
            seen["on_execute"] = kwargs["on_execute"]
            seen["on_maintenance"] = kwargs["on_maintenance"]
    ...
    async def _fake_cmd_workspace_bridge(ctx):
        ctx.msg.metadata["workspace_agent_cmd"] = "merge"
        ctx.msg.metadata["workspace_agent_input"] = "prepared merge continuation"
        return None
    ...
    monkeypatch.setattr("nanobot.heartbeat.service.HeartbeatService", _FakeHeartbeatService)
    monkeypatch.setattr("asyncio.start_server", _fake_start_server)
    monkeypatch.setattr("nanobot.command.workspace_bridge.cmd_workspace_bridge", _fake_cmd_workspace_bridge)

    result = runner.invoke(app, ["gateway", "--config", str(config_file)])

    assert result.exit_code == 0
    response = asyncio.run(seen["on_execute"]("/merge now"))
    assert response == "ok"
    payload = seen["process_direct"]
    assert payload["content"] == "/merge now"
    assert payload["session_key"] == "heartbeat:exec"
    assert payload["channel"] == "cli"
    assert payload["chat_id"] == "direct"
    assert callable(payload["on_progress"])
    assert payload["metadata"]["workspace_agent_cmd"] == "merge"
    assert "[HEARTBEAT EXECUTION" in payload["metadata"]["workspace_agent_input"]
    assert "prepared merge continuation" in payload["metadata"]["workspace_agent_input"]


def test_gateway_heartbeat_maintenance_probes_due_provider_routes(
    monkeypatch, tmp_path: Path
) -> None:
    config_file = _write_instance_config(tmp_path)
    config = Config()
    ...
    def _fake_probe_due_provider_routes(*, workspace):
        seen["probe_workspace"] = workspace
    ...
    monkeypatch.setattr(
        ...
        _fake_probe_due_provider_routes,
    )

    result = runner.invoke(app, ["gateway", "--config", str(config_file)])

    assert result.exit_code == 0
    asyncio.run(seen["on_maintenance"]())
    assert seen["probe_workspace"] == config.workspace_path
