...
def test_fastlane_falls_back_to_builtin_model_help_when_router_route_json_fails(
    tmp_path: Path,
) -> None:
    broken_router = MagicMock(stdout="", stderr="Traceback...", returncode=1)

    with (
        patch("nanobot.command.fastlane.WORKSPACE_ROUTER", tmp_path / "router.py"),
        patch("nanobot.command.fastlane.subprocess.run", return_value=broken_router),
    ):
        (tmp_path / "router.py").write_text("#!/bin/sh\n", encoding="utf-8")
        result = asyncio.run(try_workspace_fastlane(_msg("/model help"), "/model help"))

    assert result is not None
    assert result.metadata == {"render_as": "text"}
    assert result.content.startswith("/model [子命令]")
    assert "当前保持纯脚本" in result.content
