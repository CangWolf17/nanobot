...
def test_prepare_active_workflow_continuation_for_notes_awaiting_confirmation_sets_agent_metadata(tmp_path: Path) -> None:
...
def test_prepare_active_workflow_continuation_ignores_notes_state_when_phase_not_waiting(tmp_path: Path) -> None:
    sessions_root = tmp_path / "sessions"
    session_root = sessions_root / "ses_0001"
    session_root.mkdir(parents=True, exist_ok=True)
    (sessions_root / "control.json").write_text(
